...
function App() {
  ...
  const checkAndAwardBadges = (feedbackText, sessionCount) => {
    ...
    if (score) {
      ...
      if (s >= 90 && !earned.includes('gold'))
        newBadges.push({ id: 'gold', emoji: '🥇', name: 'Gold', desc: 'Scored 90+!' });
      else if (s >= 70 && !earned.includes('silver'))
        newBadges.push({ id: 'silver', emoji: '🥈', name: 'Silver', desc: 'Scored 70+!' });
      else if (s >= 50 && !earned.includes('bronze'))
        newBadges.push({ id: 'bronze', emoji: '🥉', name: 'Bronze', desc: 'Scored 50+!' });
    }
  ...
  const generateCompanyQuestions = async () => {
    ...
    setLoading(true);
    const res = await fetch('http://localhost:8001/company-questions', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ resume_text: resumeText, company: selectedCompany, role: selectedRole.label, language }),
    });
    const data = await res.json();
  ...
  const resetAll = () => {
    ...
    setWeakTopics(''); setShowWeakTopics(false);
    setSelectedCompany(''); setShowCompanyMode(false);
      ...
      <div style={{ maxWidth: '750px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '15px', flexWrap: 'wrap', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
            <button onClick={() => setLanguage(language === 'English' ? 'Hindi' : 'English')} title={language === 'English' ? 'Switch to Hindi' : 'Switch to English'}
          ...
          <div style={glassBox}>
            ...
            <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', marginBottom: '15px' }}>
              ...
              <button onClick={startPanelInterview} disabled={loading}
                ...
                {loading ? '⏳ Loading...' : '👥 Panel Interview'}
              </button>
              <button onClick={() => setShowCompanyMode(!showCompanyMode)} disabled={loading}
                style={{ padding: '10px 20px', background: '#FF6F00', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: isMobile ? '13px' : '15px', fontWeight: 'bold' }}>
                🏢 Company Round
              </button>
            </div>
            {showCompanyMode && (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '8px', marginBottom: '12px' }}>
                  {['Google', 'Amazon', 'Microsoft', 'TCS', 'Infosys', 'Wipro', 'Goldman Sachs', 'Startup'].map(company => (
                    <button key={company} onClick={() => setSelectedCompany(company)}
                      style={{ padding: '8px', background: selectedCompany === company ? '#FF6F00' : 'rgba(255,255,255,0.15)', border: '1px solid rgba(255,255,255,0.3)', borderRadius: '8px', color: 'white', cursor: 'pointer', fontSize: '13px' }}>
                      {company === 'Google' ? '🔵' : company === 'Amazon' ? '🟠' : company === 'Microsoft' ? '🟦' : company === 'TCS' ? '🇮🇳' : company === 'Infosys' ? '💼' : company === 'Wipro' ? '🌐' : company === 'Goldman Sachs' ? '💰' : '🚀'} {company}
                    </button>
                  ))}
                </div>
                <button onClick={generateCompanyQuestions} disabled={loading || !selectedCompany}
                  style={{ padding: '10px 20px', background: '#FF6F00', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', width: '100%' }}>
                  {loading ? '⏳ Generating...' : `🏢 Generate ${selectedCompany || 'Company'} Questions`}
                </button>
              </div>
            )}
          </div>
        ...
        {step >= 3 && !panelMode && (
          <div style={glassBox}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '10px', flexWrap: 'wrap' }}>
              ...
              {!timerActive ? (
                <button onClick={startTimer} style={{ padding: '10px 20px', background: '#185FA5', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px' }}>
                  ▶️ Start Timer
                </button>
              ) : (
                <button onClick={stopTimer} style={{ padding: '10px 20px', background: '#555', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '14px' }}>
                  ⏸️ Pause Timer
                </button>
              )}
            </div>
